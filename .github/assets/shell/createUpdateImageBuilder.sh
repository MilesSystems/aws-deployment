...
if ! diff -q ./CloudFormation/imagebuilder.yaml /tmp/latest_template.yaml > /dev/null; then
  diff --color=always -y -W 160 --suppress-common-lines ./CloudFormation/imagebuilder.yaml /tmp/latest_template.yaml
  ...
  if diff -q "$PARAMETERS_FILE" /tmp/latest_parameters.json > /dev/null; then
      ...
      diff --color=always -y -W 160 --suppress-common-lines "$PARAMETERS_FILE" /tmp/latest_parameters.json
